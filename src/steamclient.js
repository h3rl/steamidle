...
{
    constructor({username ,password, games, silent=true, rememberPassword=true})
    {
        super();
    ...
    _setupEmitters()
    {
        ...
        this.client.on("loggedOn",(details,parental)=>{
            ...
            this.client.getPersonas([this.client.steamID],(err,steamid)=>{
                if(err) this.emit("message",err)
                    ...
                    this.emit("message","Logged in as "+steamid[this.client.steamID].player_name)
                }
            })
        });

        this.client.on("steamGuard", (domain, AuthCodeCallback, lastCodeWrong) => {
            this.authCode = null;
        ...
        this.client.on("playingState",(blocked,playing)=>{
            if(blocked)
            {
                this.emit("message","User started "+playing+", idling suspended");
            }
    ...
    startidle()
    {
        ...
        this.client.requestFreeLicense(this.games,(err,grantedApps, grantedPackages)=>{
            if(err) _onError(err);
        })
